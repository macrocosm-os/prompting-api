...
m2.metric(
    "Total events",
    f'{productivity.get("total_events")/1e6:,.2f}M',
    delta=f'{productivity_24h.get("total_events")/1e6:,.2f}M (24h)',
)
m3.metric(
    "Total completions",
    f'{productivity.get("total_completions")/1e9:,.2f}B',
    delta=f'{productivity_24h.get("total_completions")/1e9:,.2f}B (24h)',
)
m4.metric(
    "Total dataset tokens",
    f'{productivity.get("total_tokens")/1e9:,.2f}B',
    delta=f'{productivity_24h.get("total_tokens")/1e9:,.2f}B (24h)',
)
...
st.plotly_chart(
    px.area(
        df_task_counts,
        y=df_task_counts.columns,
        title="Data Created by Task",
...
st.info(
    "Subnet 1 is an endlessly improving system, where miners compete to produce high quality responses to a range of challenging tasks"
)
...
with st.expander("Advanced settings"):
    c1, c2 = st.columns(2)
    remove_zero_rewards = c1.checkbox(
        "Exclude zero rewards",
        value=True,
        help="Remove completions which scored zero rewards (failed responses, timeouts etc.)",
    )
    normalize_rewards = c1.checkbox(
        "Normalize rewards",
        value=True,
        help="Scale rewards for each task to a maximium value of 1 (approx)",
    )
    show_releases = c1.checkbox(
        "Show releases",
        value=False,
        help="Add annotations which indicate when major releases may have impacted network performance",
    )
    moving_avg_window = c2.slider(
        "Moving avg. window",
        min_value=1,
        max_value=30,
        value=14,
        help="Window size to smooth data and make long term trends clearer",
    )

reward_col = "normalized_rewards" if normalize_rewards else "rewards"

df_stats = utils.get_reward_stats(
    df_events,
    exclude_multiturn=True,
    freq="1D",
    remove_zero_rewards=remove_zero_rewards,
)


task_choice_label = st.radio(
    "Select task", list(TASK_CHOICES.keys()), index=0, horizontal=True
)
task_choice = TASK_CHOICES[task_choice_label]

st.plotly_chart(
    # add fillgradient to make it easier to see the trend
    utils.plot_reward_trends(
        df_stats,
        task=task_choice,
        window=moving_avg_window,
        col=reward_col,
        annotate=show_releases,
        task_label=task_choice_label,
    ),
    use_container_width=True,
)
...
m1, m2 = st.columns(2)
ntop = m1.slider(
    "Number of top miners to display", value=10, min_value=3, max_value=50, step=1
)
entity_choice = m2.radio(
    "Select entity", utils.ENTITY_CHOICES, index=0, horizontal=True
)
...
st.plotly_chart(
    px.bar(
        ...
        x="I",
        color="I",
        hover_name=entity_choice,
        text=entity_choice if ntop < 20 else None,
