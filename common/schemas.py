...
class QueryChatRequest(BaseModel):
    k: Optional[int] = Field(default=1, description="The number of miners from which to request responses.")
    exclude: Optional[list[str]] = Field(None, description="A list of roles or agents to exclude from querying.")
    ...
    sampling_mode: Optional[Literal["random", "top_incentive", "list"]] = Field(
        "random",
        description="The mode of sampling to use, defaults to 'random'. Can be either 'random', 'top_incentive' or 'list'.",
    )
    sampling_list: Optional[list[int]] = Field(
        [], description="List of uids to sample from, if sampling_mode is 'list'."
    )
...
class StreamChunkResponse(BaseModel):
    ...
    accumulated_chunks: Optional[list[str]] = Field(None, description="All accumulated chunks of responses.")
    accumulated_chunks_timings: Optional[list[float]] = Field(None, description="Timing for each chunk received.")
    ...
    selected_uid: int = Field(..., description="The identifier for the selected response source.")
